...
function addBonusAmmenity() {
  ...
  let url2 = '/addamenity/' + accountNumL1 + '/' + newAmmenity;
...
function addReview() {
 ...
 /* if ((tName == "") || (rreview == "")) {
  ...
  console.log(tName)
  let url3 = '/addreview/' + tName + '/' + rreview;
...
function bookProperty() {
  ...
  let url4 = `/rent/${tnum}/${propnum}/${datefrom}/${dateto}`;

  fetch(url4, { method: 'POST' })
    .then(
      function (response) {
        if (response.status != 200) {
...
function deleteResults() {
  ...
  let url5 = `/delstars/${proptodelete}`;
